...
from dices.dnd_random_grid.metrics import sum_max_three


def generate_distribution(roll_distribution, metrics, k=1000):
    # initialization
    min_score = roll_distribution.population[0]*3
    max_score = roll_distribution.population[-1]*3
...
if __name__ == '__main__':
    ...
    for roll in ('2d6p6', '3d6', '4d6dl'):
        ...
        score_distribution = generate_distribution(roll_distribution, sum_max_three, 10000000)

        with Path(score_path, f'sum_max_three_{roll}.json').open('w') as fo:
